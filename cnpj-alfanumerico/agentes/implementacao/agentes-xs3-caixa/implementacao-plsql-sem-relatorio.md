...
> **⚠️ PRIORIDADE MÁXIMA - Palavras-chave principais para busca de campos CNPJ/CPF:**  
> **As palavras-chave `CGC`, `CPF`, `ESTAB` e `DIGITO` devem ter PRIORIDADE MÁXIMA na busca e identificação de campos relacionados a CNPJ/CPF.** Estas são as palavras-chave mais importantes e devem ser buscadas em todos os contextos (campos de tabela, variáveis locais, parâmetros, colunas, etc.), mesmo quando aparecem isoladamente ou com outros prefixos/sufixos.
...
> **Solução implementada:** Regra de busca flexível que identifica campos contendo essas palavras-chave, validando contextualmente se estão relacionados a CNPJ/CPF. **REFORÇAR A BUSCA pelas palavras-chave `CGC`, `CPF`, `ESTAB` e `DIGITO`.**
...
> **⚠️ PRIORIDADE MÁXIMA - Palavras-chave principais para busca:**  
> **REFORÇAR A BUSCA** por campos que contenham as palavras-chave **`CGC`**, **`CPF`**, **`ESTAB`** ou **`DIGITO`** (ou suas variações), mesmo quando aparecem isoladamente ou com outros prefixos/sufixos. Estas são as palavras-chave mais importantes para identificar campos relacionados a CNPJ/CPF.
>
...
> - **`CGC`** ⚠️ **PRIORIDADE MÁXIMA**  
> - **`CPF`** ⚠️ **PRIORIDADE MÁXIMA**  
> - **`ESTAB`** ⚠️ **PRIORIDADE MÁXIMA**  
> - **`DIGITO`** ⚠️ **PRIORIDADE MÁXIMA**  
> - `NUMID`  
> - `CNPJ`  
> - `NR_DOCTO`  
> - `NR_CPF_CNPJ`  
...
> - `RN_DIGITO`  
>
> **Regra de busca flexível para CGC, CPF, ESTAB e DIGITO (PRIORIDADE MÁXIMA):**  
> **REFORÇAR A BUSCA** por campos que contenham as palavras-chave **`CGC`**, **`CPF`**, **`ESTAB`** ou **`DIGITO`** (ou suas variações), mesmo quando aparecem isoladamente ou com outros prefixos/sufixos. Isso é necessário porque diferentes sistemas podem usar nomenclaturas variadas para campos relacionados a CNPJ/CPF. **Estas palavras-chave devem ter PRIORIDADE MÁXIMA na busca e identificação de campos.**  
...
>   1. **PRIORIDADE MÁXIMA:** Contém exatamente uma das palavras-chave principais **`CGC`**, **`CPF`**, **`ESTAB`** ou **`DIGITO`** (ou suas variações), OU  
>   2. Contém exatamente uma das palavras-chave principais (`CNPJ`, `NUMID`, `NR_DOCTO`, `NR_CPF_CNPJ`), OU  
>   3. **PRIORIDADE ALTA:** Contém a palavra `ESTAB` (ou `ESTABELECIMENTO`) E está relacionado ao contexto de CNPJ/identificação fiscal, OU  
>   4. **PRIORIDADE ALTA:** Contém a palavra `DIGITO` (ou `DV`) E está relacionado ao contexto de CNPJ/CPF/validação fiscal.  
...
>   1. **PRIORIDADE MÁXIMA:** Começa com `v_` (ou `V_`) ou `p_` (ou `P_`) E contém **em qualquer parte do nome** uma das palavras-chave principais **`CGC`**, **`CPF`**, **`ESTAB`** ou **`DIGITO`** (ou suas variações), **independentemente de sufixos ou prefixos adicionais**, OU  
>   2. **PRIORIDADE MÁXIMA:** Começa com `v_` (ou `V_`) ou `p_` (ou `P_`) E contém **em qualquer parte do nome** uma das palavras-chave principais (`CNPJ`, `NUMID`, `NR_DOCTO`, `NR_CPF_CNPJ`, `CGC_CPF`), **independentemente de sufixos ou prefixos adicionais**, OU  
>   3. **PRIORIDADE ALTA:** Começa com `v_` (ou `V_`) ou `p_` (ou `P_`) E contém a palavra `ESTAB` (ou `ESTABELECIMENTO`) E está relacionado ao contexto de CNPJ/identificação fiscal, OU  
>   4. **PRIORIDADE ALTA:** Começa com `v_` (ou `V_`) ou `p_` (ou `P_`) E contém a palavra `DIGITO` (ou `DIGITO_VERIFICADOR`, `DV`) E está relacionado ao contexto de CNPJ/CPF/validação fiscal.  
...
1. **Inventariar ocorrências** de CNPJ/CPF no banco de dados (tabelas, colunas, constraints, triggers, procedures, funções, pacotes, views, stored procedures, validações, índices, logs, auditoria, etc.).  
   - **⚠️ PRIORIDADE MÁXIMA - Palavras-chave principais:** **REFORÇAR A BUSCA** por campos que contenham as palavras-chave **`CGC`**, **`CPF`**, **`ESTAB`** ou **`DIGITO`** (ou suas variações). Estas são as palavras-chave mais importantes e devem ter prioridade máxima na busca.  
   ...
   - **PRIORIDADE ALTA - Atenção especial:** Buscar campos que contenham **`CGC`**, **`CPF`**, **`ESTAB`** ou **`DIGITO`** (ou variações) mesmo quando não seguem os padrões exatos `NR_ESTAB` ou `RN_DIGITO`, conforme regra flexível da Seção 1. **REFORÇAR A BUSCA** por estes padrões.  
   - **PRIORIDADE ALTA - Atenção especial para variáveis locais e parâmetros:** Buscar variáveis locais que começam com `v_` e parâmetros que começam com `p_` e contenham padrões relacionados a **`CGC`**, **`CPF`**, **`ESTAB`** ou **`DIGITO`** (ex.: `v_nr_cgc_cpf_segurado`, `v_nr_estabelecimento_segurado`, `v_nr_digito_verificador`, `v_nr_cpf_retorno`, `v_nr_cpf_cnpj_clien`, `v_cpf_cnpj_cliente`, `p_nr_estab_segur_tela`), conforme regra específica da Seção 1. **REFORÇAR A BUSCA** por estes padrões.  
   - **⚠️ CRÍTICO - Busca por CGC, CPF, ESTAB e DIGITO em variáveis:** **QUALQUER variável ou parâmetro que contenha `CGC`**, **`CPF`**, **`ESTAB`**, **`DIGITO`**, `CNPJ`, `NUMID`, `NR_DOCTO` ou `NR_CPF_CNPJ` em QUALQUER parte do nome deve ser identificada e convertida**, independentemente de sufixos como `_retorno`, `_busca`, `_formatado`, etc.  
...
- **⚠️ PRIORIDADE MÁXIMA - Palavras-chave principais (REFORÇAR BUSCA):**  
  - **`CGC`** ⚠️ **PRIORIDADE MÁXIMA**  
  - **`CPF`** ⚠️ **PRIORIDADE MÁXIMA**  
  - **`ESTAB`** ⚠️ **PRIORIDADE MÁXIMA**  
  - **`DIGITO`** ⚠️ **PRIORIDADE MÁXIMA**  
  - **Estas palavras-chave devem ser buscadas com PRIORIDADE MÁXIMA em todos os contextos (campos, variáveis, parâmetros, colunas, etc.).**
...
- **PRIORIDADE ALTA - Inclusão (busca parcial/flexível - REFORÇAR BUSCA):**  
  - Campos contendo **`CGC`** (ex.: `cgc`, `nr_cgc`, `cd_cgc`, `cod_cgc`, `cgc_cpf`, `v_nr_cgc_cpf_segurado`)  
  - Campos contendo **`CPF`** (ex.: `cpf`, `nr_cpf`, `cd_cpf`, `cod_cpf`, `cpf_cnpj`, `v_nr_cpf_retorno`)  
  ...
  - **Importante:** A busca parcial deve ser validada contextualmente para garantir que o campo se refere a CNPJ/CPF e não a outros conceitos. **REFORÇAR A BUSCA** por estes padrões, especialmente **`CGC`**, **`CPF`**, **`ESTAB`** e **`DIGITO`**.  
- **⚠️ PRIORIDADE MÁXIMA - Inclusão (variáveis locais com prefixo `v_` e parâmetros com prefixo `p_` contendo CGC, CPF, ESTAB, DIGITO - REFORÇAR BUSCA):**  
  - **CRÍTICO:** **QUALQUER variável ou parâmetro que contenha `CGC`**, **`CPF`**, **`ESTAB`**, **`DIGITO`**, `CNPJ`, `NUMID`, `NR_DOCTO` ou `NR_CPF_CNPJ` em QUALQUER parte do nome deve ser identificada e convertida**, independentemente de sufixos como `_retorno`, `_busca`, `_formatado`, `_validado`, `_temporario`, `_auxiliar`, `_cliente`, `_segurado`, etc. **As palavras-chave `CGC`, `CPF`, `ESTAB` e `DIGITO` têm PRIORIDADE MÁXIMA.**  
  - Variáveis que começam com `v_` (ou `V_`) e contêm padrões relacionados a **`CGC`**, **`CPF`**, **`ESTAB`** ou **`DIGITO`** (ex.: `v_nr_cgc_cpf_segurado`, `v_nr_estabelecimento_segurado`, `v_nr_digito_verificador`, `v_nr_cpf_retorno`, `v_cpf_busca`, `v_cnpj_formatado`, `v_cpf_validado`, `v_cnpj`, `v_cpf`, `v_cgc`, `v_nr_cpf_cnpj`, `v_estab`, `v_digito`, `v_dv`, `v_nr_cpf_cnpj_clien`, `v_cpf_cnpj_cliente`)  
  - Parâmetros que começam com `p_` (ou `P_`) e contêm padrões relacionados a **`CGC`**, **`CPF`**, **`ESTAB`** ou **`DIGITO`** (ex.: `p_nr_estab_segur_tela`, `p_cnpj`, `p_cpf`, `p_cgc`, `p_nr_cpf_cnpj`, `p_cpf_entrada`, `p_cnpj_saida`)  
...
- **Variáveis locais:** ao alterar variáveis locais de `NUMBER` para `VARCHAR2`, **manter o tamanho original** se já existir (ex.: se originalmente era `v_cnpj VARCHAR2(14)`, manter `v_cnpj VARCHAR2(14)`).  
  ...
  - **⚠️ CRÍTICO - Variáveis com prefixo `v_` contendo CGC, CPF, ESTAB, DIGITO:** **QUALQUER variável local que contenha `CGC`**, **`CPF`**, **`ESTAB`**, **`DIGITO`**, `CNPJ`, `NUMID`, `NR_DOCTO` ou `NR_CPF_CNPJ` em QUALQUER parte do nome deve ser identificada e convertida**, independentemente de sufixos como `_retorno`, `_busca`, `_formatado`, `_validado`, etc. Exemplos: `v_nr_cgc_cpf_segurado`, `v_nr_estabelecimento_segurado`, `v_nr_digito_verificador`, `v_nr_cpf_retorno`, `v_cpf_busca`, `v_cnpj_formatado`, `v_nr_cpf_cnpj_clien`, `v_cpf_cnpj_cliente`, mantendo o tamanho original se já existir. **REFORÇAR A BUSCA** por estes padrões, especialmente **`CGC`**, **`CPF`**, **`ESTAB`** e **`DIGITO`**.
...
- **Caso nenhum teste seja necessário**, o agente deve apenas registrar isso internamente.
...
4. **⚠️ Verificação crítica de variáveis:** Garantir que **TODAS as variáveis locais e parâmetros que contenham `CGC`**, **`CPF`**, **`ESTAB`**, **`DIGITO`**, `CNPJ`, `NUMID`, `NR_DOCTO` ou `NR_CPF_CNPJ` em qualquer parte do nome foram identificadas e convertidas de `NUMBER` para `VARCHAR2`**, incluindo variáveis com sufixos como `_retorno`, `_busca`, `_formatado`, etc. **As palavras-chave `CGC`, `CPF`, `ESTAB` e `DIGITO` têm PRIORIDADE MÁXIMA na verificação.** Exemplos obrigatórios: `v_nr_cpf_retorno`, `v_cpf_busca`, `v_cnpj_formatado`, `p_cpf_entrada`, `v_nr_cgc_cpf_segurado`, `v_nr_estabelecimento_segurado`, `v_nr_digito_verificador`.  
...
- **⚠️ CRÍTICO:** **TODAS as variáveis locais e parâmetros que contenham `CGC`**, **`CPF`**, **`ESTAB`**, **`DIGITO`**, `CNPJ`, `NUMID`, `NR_DOCTO` ou `NR_CPF_CNPJ` em qualquer parte do nome foram convertidas de `NUMBER` para `VARCHAR2`**, incluindo variáveis com sufixos como `_retorno`, `_busca`, `_formatado`, etc. **As palavras-chave `CGC`, `CPF`, `ESTAB` e `DIGITO` têm PRIORIDADE MÁXIMA na verificação.** Exemplos: `v_nr_cpf_retorno`, `v_cpf_busca`, `v_cnpj_formatado`, `p_cpf_entrada`, `v_nr_cgc_cpf_segurado`, `v_nr_estabelecimento_segurado`, `v_nr_digito_verificador`.  
...
> **⚠️ LEMBRETE FINAL - PRIORIDADE MÁXIMA:**  
> **As palavras-chave `CGC`, `CPF`, `ESTAB` e `DIGITO` devem ter PRIORIDADE MÁXIMA em todas as buscas e identificações de campos relacionados a CNPJ/CPF.** Sempre reforçar a busca por estas palavras-chave em todos os contextos do código PL/SQL.
